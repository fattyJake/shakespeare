...
#        if ID not in member_known: member_known[ID] = []
    ...
    for k, v in ensemble.items():
        if v['classifier']:
            ...
            scores = pd.Series(scores, name=k).map(lambda x: np.tanh(np.power(x, 1/2)*4) if x < 0.012091892 else np.power(x, 1/5))
            df_vector = pd.concat([df_vector, scores], axis=1)
    del df_vector['temp']
    gc.collect()
...
#        for col in member_known[value['MEMBER']]: df_vector.at[index, col] = 0.0
    ...
    df_condition['tp_flag'] = df_condition[['MEMBER', 'HCC']].apply(lambda x: True if x[1] in member_known.get(x[0], []) else False, axis=1)
