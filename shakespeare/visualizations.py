...
def plot_coefficients(
    ...
    assert len(coefs) == len(
        variables), "ERROR: variable-coefficient size mismatch. Aborting."
    ...
    coefs, variables = (list(t) for t in zip(
        *sorted(zip(coefs, variables), reverse=True)))
    if bottom:
...
def plot_numerics(out_true, out_pred, log=False, save_name=None):
    ...
    with plt.style.context('ggplot'):
        ...
        # hist
        plt.subplot(212)
        floor = np.min(np.concatenate([out_true, out_pred], axis=0))
        ceil = np.max(np.concatenate([out_true, out_pred]))
