...
def batch_member_codes(
    ...
    date_start, date_end, file_date_lmt, mem_date_start, mem_date_end = str(
        date_start), str(date_end), str(file_date_lmt), str(mem_date_start),\
        str(mem_date_end)
    model_name = _get_model_name(model)

    if memberIDs:
        cursor.execute("CREATE TABLE #MemberList (mem_id INT)")
        cursor.execute('\n'.join(["INSERT INTO #MemberList VALUES ({})".format(
            str(member)) for member in memberIDs]))
    ...
    WHERE mem_ClientMemberID IS NOT NULL AND dateInserted BETWEEN '""" +\
        mem_date_start + """' AND '""" + mem_date_end + """'
    ORDER BY mem_id DESC"""
    if not memberIDs:
        sql = re.sub(
            r"INNER JOIN \#MemberList m ON tbm\.mem\_id = m.mem\_id",
            "",
            sql)
    ...
    IF OBJECT_ID('""" + re.sub(r"CD\_", "CARA2_Results_" + ('HIX_' if 'HHS' in\
        model_name else ''), payer) + """.dbo.MRRData') IS NOT NULL
    BEGIN
    	...
    	INTO #mrr_temp
        FROM """ + re.sub(r"CD\_",
            "CARA2_Results_" + ('HIX_' if 'HHS' in model_name else ''), payer)\
            + """.dbo.MRRData e WITH(NOLOCK)
            INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
            LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id""" + \
                f"""{mrr_queue_sql if file_date_lmt != 'None' else ''}""" + """
            WHERE e.mrr_StartDate BETWEEN '""" + date_start + """' AND '"""\
                + date_end + """'
    END"""
    sql = re.sub(
        r"WHERE (e|p)\.[a-zA-Z\_]+ BETWEEN 'None' AND 'None'",
        "",
        sql)
    ...
    FROM """ + payer + """.dbo.tbEncounter e WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbEncounterDiagnosis ed
            ON e.enc_id = ed.enc_id
        INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
    ...
    FROM """ + payer + """.dbo.tbMedicareRapsReturn e WITH(NOLOCK)
        ...
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
            ON f.fil_id = e.fil_id AND f.fil_StartDate <= '""" +\
                file_date_lmt + """'
    WHERE e.raps_DOSfrom BETWEEN '""" + date_start + """' AND '""" +\
        date_end + """'
    ...
    FROM """ + payer + """.dbo.tbEncounter e WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbEncounterCPT eCPT
            ON e.enc_id = eCPT.enc_id
        INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
    ...
    FROM """ + payer + """.dbo.tbEncounter e WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbEncounterDRG eDRG
            ON e.enc_id = eDRG.enc_id
        INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
    ...
    FROM """ + payer + """.dbo.tbEncounter e WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbEncounterHCPCS eHCPCS
            ON e.enc_id = eHCPCS.enc_id
        INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
    ...
    FROM """ + payer + """.dbo.tbEncounter e WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbEncounterProcedure eProc
            ON e.enc_id = eProc.enc_id
        INNER JOIN #Temp tp ON tp.mem_id = e.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = e.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
    ...
    FROM """ + payer + """.dbo.tbPharmacy p WITH(NOLOCK)
        INNER JOIN """ + payer + """.dbo.tbPharmacyNDC NDC
            ON p.pha_id = NDC.pha_id
        INNER JOIN #Temp tp ON tp.mem_id = p.mem_id
        LEFT JOIN """ + payer + """.dbo.tbPractitioner pra
            ON pra.pra_id = p.pra_id
        INNER JOIN """ + payer + """.dbo.tbFile f WITH(NOLOCK)
            ON f.fil_id = p.fil_id AND f.fil_StartDate <= '""" +\
                file_date_lmt + """'
    WHERE p.pha_ServiceDate BETWEEN '""" + date_start + """' AND '""" +\
        date_end + """'
    UNION
    ...
    sql = re.sub(
        r"INNER JOIN " +
        payer +
        r"\.dbo\.tbFile f WITH\(NOLOCK\) ON f\.fil_id = (e|p)\.fil_id AND f\."\
            r"fil_StartDate <= 'None'",
        "",
        sql)
    ...
    if get_client_id:
        ...
        table = list(set([(i[0], i[2], i[3], i[5] + '-' + i[6])
                          for i in table]))
    return table
