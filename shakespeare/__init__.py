...
def _create_df(table):
    ...
    SPECIALISTS = {
        2, 3, 4, 5, 7, 10, 11, 14, 15, 16, 17, 18, 21, 22, 23, 24, 25, 27, 29,
        30, 31, 35, 37, 39, 40, 41, 42, 43, 44, 45, 47, 50, 51, 52, 59, 60, 62,
        64, 65, 66, 67, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 82, 84, 88,
        89, 90, 92, 93, 95, 100, 102, 103, 106, 109, 111, 113, 118, 119, 120,
        121, 122, 123, 124, 125, 126, 127, 133, 135, 136, 140, 142, 143, 144,
        145, 146, 147, 148, 149, 150, 152, 153, 156, 157, 158, 159, 160, 164,
        165, 166, 171, 172, 173, 174, 175, 176, 177, 179, 180, 181, 182, 186,
        187, 188, 189, 191, 192, 195, 196, 198, 199, 202, 206, 208, 209, 210,
        212, 214, 215, 217, 220, 221, 222, 223, 224, 228, 230, 231, 232, 233,
        234, 237, 998, 999
    }
...
def _get_model_name(model):
    ...
    try:
        ...
        pass
        raise ValueError(
            "{} does not have a corresponding SQL table. Not building this model.".format(
                model
            )
        )
...
def _update_mappings(model):
    ...
    if "CMS" in model_name:
        ...
        mapping_dict = cursor.execute(
            ...
            WHERE mdst_ModelSubTypeID = 1 AND mv_modelversionID = {}""".format(
                model
            )
        ).fetchall()
        ...
        mapping_dict = cursor.execute(
            ...
            WHERE mv_modelversionID = {}""".format(
                model
            )
        ).fetchall()
    ...
    if not mapping_dict:
        raise ValueError(
            "Empty mapping table for {}, aborting training session.".format(
                model
            )
        )
