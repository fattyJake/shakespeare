...
for module in [
    ...
    if not module_exists(module):
        raise ImportError(
            '\nRequired shakespeare dependencies not detected. '
...
def detect(
    ...
    if not memberID_list:
        memberID_list = list({tup[0] for tup in fetch_db.get_members(
    ...
    if not os.path.exists(
        ...
        if auto_update:
            ...
            raise FileNotFoundError(
                f'Model version not exist! Please use `shakespeare.update'
                '({model}, {year})` to train ML for Model {model}.')
    ...
    if len(memberID_list) <= 40000:
        ...
        memberID_list = [memberID_list[i:i + 40000]
...
def detect_members(
    ...
    table : list of tuples
        list of tuples of format [(mem_id, enc_id, code)]; e.g.
    ...
    df_condition = pd.melt(
        df_vector, id_vars=['MEMBER'], value_vars=[
            i for i in df_vector.columns if i.startswith('HCC')])
    df_condition.columns = ['MEMBER', 'HCC', 'CONFIDENCE']
    df_condition['CONFIDENCE_NORM'] = df_condition['CONFIDENCE'].map(
        lambda x: np.tanh(
            np.power(x, 1 / 2) * 4) if x < 0.012091892 else np.power(
                x, 1 / 5))
    df_condition = df_condition.loc[df_condition['CONFIDENCE_NORM']
                                    >= threshold, :]
    ...
    if get_indicators:
        ...
        del df_vector
        gc.collect()
        filtered_condtions = df_condition.groupby(
            'MEMBER')['HCC'].agg(list).to_dict()
        df_member['FEATURE_IMPORTANCE'] = np.nan
        df_member['FEATURE_IMPORTANCE'] = df_member['FEATURE_IMPORTANCE']\
            .astype(object)
        ...
        for HCC in indicators.keys():
            ...
            for idx, member in df_HCC_member[['MEMBER']].iterrows():
                ...
                coef_dict = dict(zip([variables[index]
                                      for index in i],
...
def _create_df(table):
    ...
    specialists = {2, 3, 4, 5, 7, 10, 11, 14, 15, 16, 17, 18, 21, 22, 23, 24,
                   25, 27, 29, 30, 31, 35, 37, 39, 40, 41, 42, 43, 44, 45, 47,
                   50, 51, 52, 59, 60, 62, 64, 65, 66, 67, 69, 70, 71, 72, 73,
                   74, 75, 76, 77, 78, 79, 82, 84, 88, 89, 90, 92, 93, 95, 100,
                   102, 103, 106, 109, 111, 113, 118, 119, 120, 121, 122, 123,
                   124, 125, 126, 127, 133, 135, 136, 140, 142, 143, 144, 145,
                   146, 147, 148, 149, 150, 152, 153, 156, 157, 158, 159, 160,
                   164, 165, 166, 171, 172, 173, 174, 175, 176, 177, 179, 180,
                   181, 182, 186, 187, 188, 189, 191, 192, 195, 196, 198, 199,
                   202, 206, 208, 209, 210, 212, 214, 215, 217, 220, 221, 222,
    ...
    if df_member.shape[1] == 5:
        ...
        df_member = df_member.groupby(['MEMBER', 'MEMBER_CLIENT_ID', 'CODE'])[
            'PRA_ID', 'SPEC_ID'].agg(list).reset_index()
        df_member['SPEC_ID'] = df_member['SPEC_ID'].map(
            lambda x: [True if spec in specialists else False for spec in x])
        df_member['PRA_ID'] = df_member.apply(lambda x: ','.join(
            [str(pra) for i, pra in enumerate(x['PRA_ID'])
                if x['SPEC_ID'][i]]), axis=1)
        df_member = df_member.groupby(['MEMBER', 'MEMBER_CLIENT_ID'])[
            'CODE', 'PRA_ID'].agg(list).reset_index()
    if df_member.shape[1] == 4:
        df_member.columns = ['MEMBER', 'PRA_ID', 'SPEC_ID', 'CODE']
        df_member['PRA_ID'] = df_member['PRA_ID'].fillna(-1)
        df_member['PRA_ID'] = df_member['PRA_ID'].astype(int)
        df_member = df_member.groupby(['MEMBER', 'CODE'])[
            'PRA_ID', 'SPEC_ID'].agg(list).reset_index()
        df_member['SPEC_ID'] = df_member['SPEC_ID'].map(
            lambda x: [True if spec in specialists else False for spec in x])
        df_member['PRA_ID'] = df_member.apply(lambda x: ','.join(
...
def _get_coef(classifier):
    ...
    if 'CalibratedClassifierCV' in str(classifier.__str__):
        coefs = [xgb_coef.coef(c.base_estimator)
...
def _get_model_name(model):
    ...
    try:
        ...
        pass
        raise ValueError(
            '{} does not have a corresponding SQL table. Not building this'\
                'model.'.format(model))
...
def _update_mappings(model):
    ...
    if 'CMS' in model_name:
        ...
        mapping_dict = cursor.execute("""
            ...
            WHERE mdst_ModelSubTypeID = 1 AND mv_modelversionID = {}"""\
            .format(model)).fetchall()
        ...
        raise ValueError(
            'The package only support CMS, HHS and CDPS, got {} instead.'\
                .format(model_name))

    if not mapping_dict:
        raise ValueError(
            "Empty mapping table for {}, aborting training session."\
                .format(model))
    ...
    variables = pickle.load(
        open(
            os.path.join(
                ...
                r'variables',
                'variables_{}'.format(model)),
            "rb"))  # list vars as (codetype + '-' + evidencecode)
    member_conditions = {mem: set(itertools.chain.from_iterable(
        [mapping[c] for c in codes if c in mapping]))\
            for mem, codes in member_codes.items()}
    member_vectors = {mem: vectorizers.build_member_input_vector(
        codes, variables) for mem, codes in member_codes.items()}
